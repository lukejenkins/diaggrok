...
Names by source (from sources/DIAG_LOG_INDEX.yaml):
    canonical: LOG_GNSS_GAL_E1_MEASUREMENT_REPORT_C
    ...
    disproven_names (WRONG - explicitly refuted, do NOT use):
        LOG_EVENT_MTP_FORMAT_STORE_DONE
            source: qxdm_3_12_714_2017_diag_log_codes (refuted by: qualcomm_firmware_f3_log_emission_oracles)
            reason: Community QXDM-2017 name for an unrelated subsystem (MTP format-store event). Firmware mc_gnssmeasreport.c proves 0x1886 is a Galileo E1 GNSS measurement report - wrong, not merely older.
...
class Diag0x1886:
    ...
    Canonical name is ``LOG_GNSS_GAL_E1_MEASUREMENT_REPORT_C`` from the firmware
    QSR4 F3 oracle (``mc_gnssmeasreport.c``: ``Log packet allocation failed for
    GAL E1 0x1886``), which outranks the community zukgit binding of the generic
    ``LOG_GNSS_GAL_MEASUREMENT_REPORT`` name to this code (#N Tier-1).  The
    firmware carries the generic ``LOG_GNSS_GAL_MEASUREMENT_REPORT_C`` symbol
    ...
    ``_C`` is Qualcomm's standard log-code enum-constant suffix (every ``LOG_*``
